...
import { X, Search, Plus, Heart, BookOpen, Star, Loader2, Globe } from 'lucide-react';
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 border border-pink-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden z-10 my-8 text-black dark:text-zinc-100"
        >
          {/* Header */}
              ...
              <div className="space-y-4">
                ...
                {!isSearching && searchResults.length > 0 && (
                  <div className="space-y-2 mt-4">
                    <div className="text-xs font-semibold text-gray-700 dark:text-zinc-300 px-1 flex items-center justify-between">
                      <span>Bulunan Sonuçlar ({searchResults.length})</span>
                      <span className="text-[11px] text-gray-500 dark:text-zinc-400">Eklemek istediğiniz kitabın üstüne tıklayın</span>
                    </div>
                    ...
                    {searchResults.map((item) => (
                      <div
                        ...
                        className="flex items-center gap-3 p-3 rounded-xl bg-pink-50/50 dark:bg-zinc-800/50 hover:bg-pink-100/70 dark:hover:bg-zinc-800 border border-pink-200/90 dark:border-zinc-700 hover:border-sky-300 cursor-pointer transition group shadow-xs"
                      >
                        {item.coverUrl ? (
                          <img
                            src={item.coverUrl}
                            alt={item.title}
                            className="w-12 h-16 object-cover rounded-lg shadow-xs border border-pink-200 dark:border-zinc-700 shrink-0"
                            onError={(e) => {
                              e.target.style.display = 'none';
                            }}
                          />
                        ) : (
                          <div className="w-12 h-16 rounded-lg bg-pink-100 dark:bg-zinc-700 flex items-center justify-center text-gray-500 dark:text-zinc-400 shrink-0">
                            <BookOpen className="w-5 h-5" />
                          </div>
                        )}

                        <div className="flex-1 min-w-0">
                          <h4 className="text-sm font-bold text-black dark:text-zinc-100 group-hover:text-sky-800 dark:group-hover:text-sky-300 truncate">
                            {item.title}
                          </h4>
                          <p className="text-xs text-gray-700 dark:text-zinc-400 font-medium truncate mt-0.5">
                            {item.author}
                          </p>
                          <div className="flex items-center gap-2 mt-1 text-[11px] text-gray-600 dark:text-zinc-400">
                            {item.year && <span>{item.year}</span>}
                            {item.pages && <span>• {item.pages} sayfa</span>}
                            <Badge variant="outline" className="text-[9px] py-0 px-1.5 h-4 text-gray-500 dark:text-zinc-400">
                              {item.source}
                            </Badge>
                          </div>
                        </div>
                ...
                {!isSearching && searchResults.length === 0 && searchQuery && (
                  <div className="py-8 text-center text-gray-600 dark:text-zinc-400 text-xs font-medium">
                    <p className="font-bold text-black dark:text-zinc-100 text-sm mb-1">Sonuç bulunamadı</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-black dark:text-zinc-200">
                      Kitap Adı *
                    </label>
                    <Input
                      type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      className="w-full h-9 bg-white dark:bg-zinc-800 border border-pink-200 dark:border-zinc-700 rounded-xl px-3 py-1.5 text-sm text-black dark:text-zinc-100 focus:outline-none focus:border-[#89CFF0] focus:ring-2 focus:ring-[#89CFF0]/30 shadow-xs"
                    >
                    ...
                    <div className="flex items-center gap-1 py-1">
                      {[1, 2, 3, 4, 5].map((s) => (
                        ...
                        >
                          <Star
                            className={`w-5 h-5 ${
                              rating >= s ? 'fill-amber-400 text-amber-500' : 'text-gray-300 dark:text-zinc-600'
                            }`}
                          />
                        </button>
                      ))}
                ...
                <label className="flex items-center gap-2 cursor-pointer pt-1">
                  <input
                    ...
                    className="w-4 h-4 rounded border-pink-300 dark:border-zinc-700 text-sky-500 focus:ring-sky-400 bg-white dark:bg-zinc-800"
                  />
                  <span className="text-xs font-semibold text-black dark:text-zinc-200 flex items-center gap-1">
                    <Heart className="w-3.5 h-3.5 text-rose-500 fill-rose-500" /> Bu kitabı favorilerime (başyapıtlarıma) ekle
                  </span>
                </label>
